...
在完成一次热榜抓取后，运行下面的命令将跨平台标题归并为事件，先以排名、出现次数和跨平台扩散计算动量，再由 Jev 对每个事件并行判断相关性、新颖度、账号匹配、可内容化程度、差异化空间、风险、事实核查需求、目标受众与推荐角度。

```bash
# 不调用模型，检查采集、事件归并与动量预筛是否正常
python -m trendradar.jev --dry-run
...
账号画像和分数权重见 `config/jev_radar.yaml`。报告同时输出 Markdown 和 JSON 到 `output/jev-radar/<日期>/`。动量始终由代码计算；Jev 只负责无法由数值直接求出的语义判断。没有配置密钥时，命令会自动降级为动量预筛，不会伪造语义分数。

<br>

### 本地 Web 工作台

启动本地优先的工作台（仅监听 `127.0.0.1`）：

```bash
python -m trendradar.webapp
```

访问 `http://127.0.0.1:8765` 后，可以查看机会卡片、运行 Jev 评分，并在页面内配置：Jev API Key/API Base、TrendRadar 自带深度分析所用的大模型 Key/API Base/模型名、账号画像、目标受众、推荐内容角度和候选数。密钥仅保存在本机 `.env`，页面永远不会回显密钥值。
