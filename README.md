...
> **不是“再做一个 AI 写作器”，而是先判断什么值得做。**

本仓库在 TrendRadar 的国内热榜采集能力之上，加入了以 **Jev** 为核心的热点机会发现层。它将海量、短生命周期的热榜信号，压缩成少数值得优先研究、核查和创作的机会。
...
Jev 高频语义判断（多问题并行）
        ↓
分人群机会榜单 + Markdown / JSON 报告
        ↓
可选：强 LLM 深度研究、事实核查、脚本创作
```

### Jev 在这里做什么？

Jev 是本项目的 **Intelligence Filter（高频判断引擎）**，不是爬虫，也不负责写长文。对每个已聚类的热点事件，它会在一次请求中并行判断：

| 代码负责的事实计算 | Jev 负责的语义判断 |
| --- | --- |
| 跨平台扩散、出现次数、排名质量、排名变化 | 相关性、新颖度、账号匹配、可内容化程度、差异化空间、风险 |
| 标题去重与事件聚类 | 事实核查需求、推荐内容角度、各人群适配度 |
| 动量预筛与最终加权排序 | 哪些热点值得进入下一步研究 |

这样可以避免让昂贵的生成模型阅读所有热榜内容：**代码先根据可验证的实时信号筛选，Jev 再进行细粒度判断，强 LLM 只处理真正值得深挖的少量候选。**

### 本仓库新增能力

- 聚合国内热榜并将相似标题归并为同一事件。
- 以跨平台传播、持续出现、排名和排名上升计算事件动量。
- 用 Jev 对候选事件做多维评分，并输出可追溯的 JSON 与 Markdown 报告。
- 针对以下 7 类人群，分别计算适配分并生成独立排行榜：
...
- 提供仅监听本机的浅色结果看板，专注展示各人群的 Top 热点，不展示或回显密钥。

### 快速开始

```bash
# 1. 安装依赖并配置本机密钥（.env 不会被 Git 跟踪）
cp .env.example .env
...
打开 `http://127.0.0.1:8765` 查看分人群榜单。报告会保存到 `output/jev-radar/<日期>/`：Markdown 适合阅读，JSON 适合后续接入工作流。

配置见 `config/jev_radar.yaml`，可调整创作者画像、目标人群、推荐角度、候选数与加权方式。未配置 `TYPESAFE_API_KEY` 时，系统会降级为只按动量排序，不会伪造 Jev 语义分数。

> **提示：** 热点判断不是事实确认。对高风险、健康、金融、政策或争议性事件，仍应先根据“事实核查需求”回到一手来源验证，再发布内容。
...
### 2️⃣ 第二步：设置 GitHub Secrets
   ...
   **设置步骤：**
   ...
   1. **查看生成的 URL**：
      ...
      - 格式如：`https://hooks.slack.com/services/REPLACE_WITH_YOUR_WEBHOOK`
